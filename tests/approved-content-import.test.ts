...
describe("approved-content Production importer", () => {
  ...
  it("preserves explicitly authored typed specs in new snapshots and replays without rewriting them", async () => {
    const database = await importReadyDatabase();
    const manifest = signedManifest();
    const spec = {
    ...
    manifest.questions[0].answer = { spec };
    resign(manifest);
    const validated = validateApprovedContentManifest(manifest);
    expect((await runImport(database, validated)).status).toBe("applied");
    const snapshot = await database.query(
      "select snapshot_json, content_sha256 from question_versions where question_id=$1",
      [manifest.questions[0].id],
    );
    expect(snapshot.rows).toHaveLength(1);
    expect(snapshot.rows[0]).toMatchObject({
      snapshot_json: { answer: { spec } },
    });
    expect((await runImport(database, validated)).summary.questions.noOp).toBe(
      2,
    );
    expect(
      (
