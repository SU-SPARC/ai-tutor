...
async function parseMutation(request: Request): Promise<ParseResult> {
  ...
  if (body.action !== undefined) {
    if (typeof body.action !== "string" || !actionAliases[body.action]) {
      return { error: "A supported generated-question action is required.", status: 400 }
    }
  ...
  if (body.reviewStatus !== undefined) {
    if (typeof body.reviewStatus !== "string" || !isValidReviewStatus(body.reviewStatus)) {
      return { error: "reviewStatus must be a supported review status.", status: 400 }
    }
  ...
  if (body.trustLevel !== undefined) {
    if (typeof body.trustLevel !== "string" || !isValidTrustLevel(body.trustLevel)) {
      return { error: "trustLevel must be a supported trust level.", status: 400 }
    }

    if (body.trustLevel === "private_reference") {
      return {
        error: "private_reference trust cannot be written through admin question mutations.",
        status: 400,
      }
    }
  ...
  if (body.difficulty !== undefined) {
    if (typeof body.difficulty !== "string" || !isValidDifficulty(body.difficulty)) {
      return { error: "difficulty must be foundational, intermediate, or challenge.", status: 400 }
    }
...
function parseTopicId(
  ...
  if (topicId && topic && topicId !== topic) {
    return { error: "topic and topicId must refer to the same topic id.", status: 400 }
  }
...
function parseStringArray(
  ...
  for (const item of value) {
    ...
    if (!trimmed) {
      return { error: `${fieldName} cannot contain empty strings.`, status: 400 }
    }

    if (trimmed.length > MAX_TEXT_FIELD_LENGTH) {
      return { error: `${fieldName} contains an item that is too long.`, status: 400 }
    }
...
function parseMisconceptions(
  ...
  const misconceptions: NonNullable<AdminQuestionDetailUpdate["misconceptions"]> =
    []
  ...
  for (const item of value) {
    ...
    if (id.length > 100 || feedback.length > MAX_TEXT_FIELD_LENGTH) {
      return { error: "Misconception id or feedback is too long.", status: 400 }
    }

    const matchTerms =
      item.matchTerms === undefined
        ? { value: [] }
        : parseStringArray(item.matchTerms, "misconception matchTerms", MAX_MATCH_TERMS)
