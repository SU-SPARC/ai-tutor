...
Status: future-architecture proposal, reviewed 2026-09-06. The published-only
"Practice a similar problem" baseline is implemented through
`GET /api/tutor/session/[sessionId]/similar`; generated variants are not
implemented. This document designs that future extension without weakening the
professor-governed content architecture in
...
| Order of preference            | Published alternative → shared variant pool → deterministic generator from a professor-enabled pattern → LLM-parameterized generator (later phase, off by default).                       |
...
Practice workspace (client)
   ├─ GET  /api/tutor/session/[sessionId]/similar        (implemented published-only lookup)
   └─ POST /api/tutor/session/[sessionId]/similar        (future ladder; idempotencyKey body)
   ▼
Similar-practice service (server, src/lib/practice-variants/)
   ├─ 1. existing published-question finder → normal tutor session flow
...
| 1    | Published alternative       | Existing finder returns another published, available question in the same topic; uncompleted candidates are preferred | none   | `published_question`         |
...
200  { question: { questionId, topicId, difficulty, title } | null }

Future additive ladder contract:
POST /api/tutor/session/[sessionId]/similar
Body    { idempotencyKey: string (1–128 chars) }
200     { outcome: "published_question", questionId, title, reason: "same_pattern" | "same_difficulty" | "same_topic" }
...
| ------------------------------------------ | --------------------- | ------------------------------------------------- |
| `PRACTICE_VARIANTS_ENABLED`                | `false`               | Whole feature beyond published alternatives       |
...
| ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| 0     | Implemented baseline: existing published-question lookup through `GET /api/tutor/session/[sessionId]/similar`, workspace button, no generation ledger                                                                                                                                                                                              | Existing similar-published-question service and API/UI tests remain green                                                                   |
...
### Prompt 1 — Verify the implemented published-alternative baseline

```
Read docs/practice-variants.md sections 1.3, 4.1, and 6 first. Inspect the
implemented baseline in src/app/api/tutor/session/[sessionId]/similar/route.ts,
src/lib/tutor/similar-published-question.ts,
...
published questions in the same topic, prefers difficulty/pattern/shared
misconception/uncompleted signals in the implemented order, returns a minimal
four-field DTO, leaks no cross-student progress, and uses no LLM. Record any
baseline contract changes needed by later phases as additive behavior; keep GET
backward compatible.

Verify: npm run lint && npm run typecheck && npx vitest run tests/similar-published-question.test.ts tests/similar-published-question-api.test.ts tests/practice-similar-problem-action.test.ts
```
