...
| `AUTH_SESSION_SECRET`                   | Authentication   | **Yes**          | Required in every deployed OIDC environment. Must be a unique, high-entropy value of at least 32 characters, distinct from `AUTH_CLIENT_SECRET`, and not a placeholder/repeated pattern. Server-only.                                                       |
...
`APP_URL` pins Auth.js callback and session-action URLs to the intended
environment. Use the exact environment origin and do not configure a parent
cookie domain. Authentication sessions are host-only, so Preview and Production
do not share browser sessions across sibling hostnames.
