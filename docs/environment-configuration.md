...
| `AI_REQUEST_TIMEOUT_MS`                    | AI/reliability   | No      | Required when AI is enabled in Staging or Production. Must be between `1000` and `30000`; defaults to `25000` outside strict environments.                                                                                                                  |
| `AI_USAGE_HMAC_SECRET`                     | AI/privacy       | **Yes** | Required with at least 32 characters when AI is enabled in Staging or Production. Used only for HMAC-derived usage, reservation, and cache identifiers. Server-only.                                                                                        |
| `MAX_LLM_OUTPUT_TOKENS`                    | AI/cost          | No      | Required integer from `64` through `400` when AI is enabled in Staging or Production. Defaults to `400` only outside strict environments.                                                                                                                   |
| `AI_LLM_MAX_REQUESTS_PER_SESSION`          | AI/cost          | No      | Maximum cache-miss LLM requests for one tutor session. Defaults to `3`; allowed range is `1`–`20`.                                                                                                                                                          |
| `AI_LLM_MAX_REQUESTS_PER_STUDENT_QUESTION` | AI/cost          | No      | Maximum cache-miss LLM requests for one HMAC-scoped student and immutable question version across sessions. Defaults to `6`; allowed range is `1`–`100`.                                                                                                    |
| `AI_LLM_DAILY_REQUEST_LIMIT`               | AI/cost          | No      | Optional UTC-day pilot allowance per HMAC-scoped student. When present it must be `1`–`10000`; when absent there is no daily allowance, while session/question/burst limits still apply.                                                                    |
| `AI_LLM_BURST_MAX_REQUESTS`                | AI/abuse         | No      | Maximum cache-miss LLM requests for one HMAC-scoped student during the burst window. Defaults to `4`; allowed range is `1`–`30`.                                                                                                                            |
| `AI_LLM_BURST_WINDOW_SECONDS`              | AI/abuse         | No      | Durable LLM burst window. Defaults to `60` seconds; allowed range is `10`–`3600`.                                                                                                                                                                           |
