...
identifier. An authorized instructor nevertheless sees the students' usernames on the
Students page, because a class list is read by username, and can ask for one
student's name and email address from the detail page when a course requires
reaching them. Usernames are resolved for the signed-in professor at each
visit and every display is recorded; they are never written into the
analytics.

The username is Clerk's own `username` field: the Clerk instance requires one
at sign-up, the application stores none of its own, and nothing is ever
assembled from the email address's local part, the name, or the subject. An
account that holds no username reads "Username unavailable".
...
The Students page (`/professor/students`, both views) shows usernames. They
are resolved on the server while the page renders, after
`requireAnalyticsAccess`, so a signed-out visitor is redirected and a student
is refused before any username is read; the population is the page's own,
never the caller's to choose. Each student carries `status` and, when
identified, `username` only: the display name and email address stay behind
the single reveal, and the page reads nothing else from what Clerk returns.
Clerk is read in batches of at most a hundred accounts through the user
listing, filtered by id; an id the listing does not return is `unlinked`, and
a batch that fails leaves its accounts `unavailable` without affecting the
others. Nothing is fetched or stored by the browser: the usernames arrive as
part of the rendered page, and every link into the page has prefetching
disabled so a hover never renders, and records, a page the professor did not
open.
...
auditor can tell which page view showed a username from a deliberate reveal
of one student; every render of the page, including a reload, is recorded. The
username, name, and email address that were shown are not recorded.
...
page every student is shown "Username temporarily unavailable", and with no
username to order by the by-topic groups keep their pseudonymous order. An
unauditable
...
The Students page shows its students' usernames in both views, with no action
required. In the **Activity** view each row shows the student's username
above their code, in the table's own order (last active, accuracy, attempts,
or sessions); the search box still matches only the student code. In the
**By topic** view every student is listed under each syllabus topic they have
practised, in syllabus order, and students with no practised topic under "No
topic practice yet"; within each group students are ordered by username A–Z,
ignoring case. An identified account without a username reads "Username
unavailable" and follows every account that has one; students with no
account to show — anonymous, unlinked, or unavailable — follow those, in
pseudonym order. A student whose account no longer exists reads "No longer
has an account"; one who practised without signing in reads "No account:
practised without signing in".
