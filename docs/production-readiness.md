...
- Usage controls include input/output limits, one in-flight generation per
  ...
  response caching. Durable cache-miss request limits apply per session and per
  student/question, with an optional UTC-day pilot allowance and a durable
  student burst limit.
...
- LLM usage keys are HMAC-derived, and production LLM execution fails closed
  without durable usage configuration. Cache-miss requests have durable
  session, student/question, and burst limits; an optional daily pilot limit is
