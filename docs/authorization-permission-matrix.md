...
| `POST /api/tutor/session/[sessionId]/similar`    | Owning student/professor/anonymous        | Requires a completed owned session; the server selects an explicitly enabled, currently eligible Reserve question and creates the version-pinned optional-practice session. The client never supplies a Reserve question ID. |
...
| `POST /api/professor/questions/[id]/reserve`     | Professor                                 | Toggles audited Save for later intent and the separate “Allow as similar-problem practice” permission on the exact approved/unpublished working version; never publishes.                                                    |
...
| `POST /api/professor/students/[studentKey]/identity` | Professor                             | Explicit identity reveal for one pseudonymous key. Returns the account display name and primary email address only; no other profile field, no provider subject, and no analytics record is written.                          |
