...
| Analytics              | Instructor analytics, export v2                             | Separate professor section; export v3 `practiceVariants` block                                              |
...
| 2     | Professor policies page, pool view, variant detail, quarantine/restore, promote-to-draft, prefill; analytics section; export v3 `practiceVariants`; dashboard "extra practice" line; resume list; prune script                                                                                                                                     | Professor can govern everything without SQL; export consumers see `schemaVersion: 3`                                                        |
...
### Prompt 8 — Analytics, export v3, dashboard extra-practice, prune script
...
Export: version 2 is already used by the tutor-session engagement semantic change. Bump the pilot export schemaVersion to 3, add a practiceVariants block with the same pseudonym rules and no content text, and update docs/pilot-analytics-export.md and its version guidance.
...
Invariant tests: assert that a student with only variant sessions has zero questions, zero completed questions, and zero attempts in getStudentProgress, instructor list, cohort analytics, and export v3 cohort/participant/topic/question sections.
